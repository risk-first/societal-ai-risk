...
module.exports = async function gemaraMetadataPlugin(context, options) {
  return {
    ...
    async loadContent() {
      ...
      // Find metadata.yaml in root and docs directories
      const searchPaths = [
        path.join(siteDir, 'metadata.yaml'),
        path.join(siteDir, 'metadata.yml'),
      ];
      
      // Also search in docs subdirectories
      ...
      const metadataFiles = [
        ...searchPaths.filter(p => fs.existsSync(p)),
        ...findMetadataFiles(siteDir),
      ];
